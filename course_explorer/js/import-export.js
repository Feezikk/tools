// =============================================================================
// IMPORT-EXPORT.JS
// Building export artifacts (Excel workbooks, JSON, etc.) from the current course/curriculum-map data.
// =============================================================================
...
window.exportSearchResults = function exportSearchResults() {
    if (!SearchState.results?.length) { showToast("No results to export.", "error"); return; }
    ...
    try {
        ...
        const sumRows = [
            ...
            [{ v: "Search Query",            s: sHeaderLeft }, { v: SearchState.query,                               s: sText   }],
            [{ v: "Total Hits (Matches)",    s: sHeaderLeft }, { v: totalHits,                                        s: sNumber }],
            [{ v: "Total Files Affected",    s: sHeaderLeft }, { v: SearchState.results.length,                      s: sNumber }],
            [{ v: "HTML View Mode",          s: sHeaderLeft }, { v: SearchState.isHtmlMode ? "Enabled" : "Disabled", s: sText   }],
            [{ v: "Exclude Standards",       s: sHeaderLeft }, { v: SearchState.excludeStandards ? "Yes" : "No",      s: sText   }],
            [{ v: "Exclude ML",              s: sHeaderLeft }, { v: SearchState.excludeML        ? "Yes" : "No",      s: sText   }],
            [{ v: "Exclude Notes",           s: sHeaderLeft }, { v: SearchState.excludeNotes     ? "Yes" : "No",      s: sText   }],
                ...
                .forEach(res => {
                    ...
                    if (res.isIdMatch && !allMatches.length) {
                        ...
                        allMatches.forEach(m => {
                            const pad     = SearchState.isHtmlMode ? 80 : 60;
                            const mLen    = SearchState.query.length;
        ...
        const safeQuery = SearchState.query.replace(/[^a-z0-9\-_]/gi, '_').substring(0, 30) || "Search";
...
window.exportStdsSearchResults = function exportStdsSearchResults() {
    if (!StandardsState.searchResults?.length) { showToast("No results to export.", "error"); return; }
    if (typeof XLSX === 'undefined')        { showToast("Excel library failed to load.", "error"); return; }
    ...
    try {
        ...
        let totalHits = StandardsState.searchResults.length;
        const moduleBreakdown = {}, lessonBreakdown = {}, resultsByModule = {};
        const uniqueStatesSet = new Set();

        StandardsState.searchResults.forEach(res => {
            const loc = parseLocDetails(res.locationId, res.breadcrumb);
        ...
        const viewDisplay  = { lesson: "Lesson View (Aggregated)", table: "Table View (Aggregated)" }[StandardsState.view]
                             || "Page View (All Instances)";
        const modDisplay   = StandardsState.activeModules.size === 0 ? "All Modules"       : `${StandardsState.activeModules.size} Selected`;
        const stateDisplay = StandardsState.activeStates.size  === 0 ? "All States/Groups" : `${StandardsState.activeStates.size} Selected`;
        ...
        StandardsState.searchResults.forEach(std => {
...
window.exportMediaReport = function exportMediaReport() {
    const expVideos = courseVideos.filter(m =>
        ...
        (MediaState.filterTypes.size === 0 || MediaState.filterTypes.has(m.kalturaType))
    );
    ...
    const expImages = courseImages.filter(img => {
        ...
        if (MediaState.filterTypes.size > 0 && !MediaState.filterTypes.has(img.fileType)) return false;
        ...
        // Quality Check: File Name Errors
    ...
    const expGlossary = courseGlossary.filter(g => {
        if (MediaState.activeModules.size > 0 &&
        ...
        if (GlossaryState.activeLetters.size > 0) {
            const letter = /[A-Z]/.test(g.title.charAt(0).toUpperCase())
                ? g.title.charAt(0).toUpperCase() : '#';
            if (!GlossaryState.activeLetters.has(letter)) return false;
        }
        return true;
    });
...
window.generateSpreadsheet = function generateSpreadsheet() {
    ...
    try {
        ...
        const exportStdGroups = StandardsState.activeGroups.size > 0
            ? Array.from(StandardsState.activeGroups).sort()
        ...
        const filename = customCourseName
            ? `${customCourseName.replace(/[^a-z0-9]/gi, '_')}_Curriculum_Maps.xlsx`
            : "Course_Curriculum_Maps.xlsx";
        downloadWorkbook(wb, filename);
