...
window.runStdsSearch = function runStdsSearch() {
    ...
    const updateToggleState = (resultsArr) => {
        ...
        StandardsState.sharedColors.clear();
        StandardsState.colorIndexTracker = 0;
        if (isGrouped && canBeGrouped) {
            codeStateMap.forEach((states, code) => {
                if (states.size > 1) {
                    StandardsState.sharedColors.set(code, SHARED_COLORS[StandardsState.colorIndexTracker++ % SHARED_COLORS.length]);
                }
            });
        }
    };

    if (!query && StandardsState.activeModules.size === 0 && StandardsState.activeStates.size === 0) {
        grid.innerHTML                       = '';
        elements.exportStdsBtn.style.display = 'none';
        elements.stdsStats.style.display     = 'none';
        StandardsState.searchResults             = [];
        updateToggleState([]);
        return;
    }
    ...
    let results = courseStandardsList.filter(std => {
        if (StandardsState.activeModules.size > 0 && !StandardsState.activeModules.has(std.locationId.split('.')[0])) return false;
        if (StandardsState.activeStates.size  > 0 && !StandardsState.activeStates.has(std.group))  return false;
        if (query) {
    ...
    if (StandardsState.view === 'lesson' || StandardsState.view === 'table') {
    ...
    StandardsState.searchResults = results;
    grid.innerHTML           = '';
    ...
    StandardsState.exactPattern = query ? new RegExp(`(${escapeRegExp(query)})`, 'gi') : null;
    StandardsState.fuzzyPattern = fuzzyPattern;
    StandardsState.query        = query;
    ...
    StandardsState.groupedResults = Array.from(groupedResults.values());
    StandardsState.renderCount    = 0;

    if (StandardsState.view === 'table') renderStdsTable();
...
window.renderStdsBatch = function renderStdsBatch() {
    ...
    const isGrouped = document.getElementById('stds-group-toggle')?.checked;
    const start     = StandardsState.renderCount;
    const end       = Math.min(start + STDS_BATCH_SIZE, StandardsState.groupedResults.length);

    for (let i = start; i < end; i++) {
        const group = StandardsState.groupedResults[i];
    ...
    StandardsState.renderCount = end;

    if (StandardsState.renderCount < StandardsState.groupedResults.length) {
        const remaining = StandardsState.groupedResults.length - StandardsState.renderCount;
        ...
        btn.innerHTML =
            ...
            `(Showing ${StandardsState.renderCount} of ${StandardsState.groupedResults.length})`;
...
window.renderStdsTable = function renderStdsTable() {
    const grid = document.getElementById('stds-results-grid');
    grid.innerHTML = '';
    if (!StandardsState.groupedResults.length) return;

    const sortedStates = Array.from(
        new Set(StandardsState.searchResults.map(s => s.group))
    ).sort();
    ...
    StandardsState.groupedResults.forEach(group => {
        ...
        sortedStates.forEach(state => {
            ...
            if (stdsByState[state]) {
                ...
                stdsByState[state].forEach(std => {
                    let inlineStyle = '';
                    if (StandardsState.sharedColors.has(std.code)) {
                        const c = StandardsState.sharedColors.get(std.code);
...
window.openStdCellModal = function openStdCellModal(encCellData, encGroup, encLocationId) {
    ...
    cellData.forEach((std, index) => {
        ...
        if (StandardsState.query) {
            if (StandardsState.exactPattern) {
                StandardsState.exactPattern.lastIndex = 0;
                displayCode = displayCode.replace(
                    StandardsState.exactPattern,
                    '<span class="highlight">$1</span>'
                );
            }
            let activePattern = null;
            if (StandardsState.fuzzyPattern) {
                StandardsState.fuzzyPattern.lastIndex = 0;
                if (StandardsState.fuzzyPattern.test(std.statement)) {
                    activePattern = StandardsState.fuzzyPattern;
                }
            }
            if (!activePattern && StandardsState.exactPattern) {
                activePattern = StandardsState.exactPattern;
            }
            if (activePattern) {
...
function getHighlightedStd(code, statement) {
    ...
    if (StandardsState.query) {
        ...
        if (StandardsState.fuzzyPattern) {
            StandardsState.fuzzyPattern.lastIndex = 0;
            if (StandardsState.fuzzyPattern.test(statement)) {
                StandardsState.fuzzyPattern.lastIndex = 0;
                displayState = displayState.replace(
                    StandardsState.fuzzyPattern,
...
// CURRICULUM MAP VIEW — STATE / GROUP FILTER DROPDOWN
...
window.renderStdFilterDropdown = function renderStdFilterDropdown() {
    ...
    const groupsHtml = Array.from(courseStandardGroups).sort().map(g =>
        buildFilterCheckboxHTML(
            ...
            escapeHtml(g),
            StandardsState.activeGroups.has(g)
        )
    ).join('');
...
window.toggleStdGroup = makeSetMembershipToggle(StandardsState.activeGroups, () => {
    updateStdFilterBtnText();
    renderModuleDetail(false);
    saveAppState();
});

window.toggleAllStdGroups = makeSetAllToggle(
    StandardsState.activeGroups,
...
window.updateStdFilterBtnText = function updateStdFilterBtnText() {
    ...
    if (!btn) return;
    if (StandardsState.activeGroups.size === 0) {
        btn.innerHTML = `States <span id="svg-arrow-down-map-state">${SVGS.arrowDown}</span>`;
        btn.classList.remove('filtered');
    } else {
        btn.innerHTML = `${StandardsState.activeGroups.size} States <span id="svg-arrow-down-map-state">${SVGS.arrowDown}</span>`;
...
window.toggleFolderDetails = function toggleFolderDetails(event, btn) {
    ...
    const container = btn.closest('.map-lesson') ||
                      btn.closest('.standards-box') ||
                      btn.closest('.map-page');
    if (!container) return;
