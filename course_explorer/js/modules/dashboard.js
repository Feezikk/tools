// =============================================================================
// DASHBOARD.JS
// Renders the main module/lesson curriculum map dashboard grid and module detail view.
// =============================================================================
...
function renderModuleDetail(autoExpand = true) {
    const container = elements.mapDetailContent;
    const openLessonIds = Array.from(container.querySelectorAll('details.map-lesson[open]')).map(d => d.id);
...
function renderLessonBlock(lesson, modNum) {
    ...
    details.className = (contentPass && standardsPass) ? 'map-lesson lesson-complete' : 'map-lesson lesson-has-gaps';
    ...
    contentDiv.className = 'map-lesson-content';
...
function renderPageBlock(page, parentID, reqStd) {
    ...
    div.className = (hasStandards || !reqStd) ? 'map-page page-complete' : 'map-page page-incomplete';
...
function getGroupedAndSortedStandards(standards) {
    ...
    const filtered = StandardsState.activeGroups.size > 0 ? standards.filter(s => StandardsState.activeGroups.has(s.group)) : standards;
    const groups = {};
    filtered.forEach(s => {
