...
async function handleBatchScan(files) {
    ...
    [
        ...
        elements.mediaArea, elements.dashboardGrid, elements.mapTopBar,
        elements.mapDetailArea, elements.viewSwitcher, elements.setupArea,
    ...
    const extractTitle = async (fMap, fallbackName) => {
        ...
        if (settingsFile) {
            try {
                ...
                console.warn(`Course Explorer: found settings.json for "${fallbackName}" but could not parse it as JSON — falling back to the folder name as the course title.`, e);
            }
        }
        return fallbackName;
    };
