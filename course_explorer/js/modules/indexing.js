...
async function runIndexingAndShowUI(wasRefreshed) {
    try {
        ...
        courseTree          = {};
        validModules        = [];
        ReadabilityState.hierarchy = null;
        courseVideos        = [];
        courseAudio         = [];
        courseInteractives  = [];
        courseImages        = [];
        courseDocuments     = [];
        MediaState.downloadsView = 'linked';
        MediaState.downloadsSearchQuery = '';
        MediaState.cachedUnlinkedDocuments = null;
        MediaState.scrollPositions = {};
        MediaState.imageLoadedCount = 0;
        if (typeof revokePendingImageBlobUrls === 'function') revokePendingImageBlobUrls();
        ...
        StandardsState.activeModules.clear();
        StandardsState.activeStates.clear();
        SearchState.activeModules.clear();
        ...
        SearchState.query                    = "";
        SearchState.highlightRegex           = null;
        ...
        StandardsState.query        = "";
        StandardsState.exactPattern = null;
        StandardsState.fuzzyPattern = null;
        StandardsState.groupedResults      = [];
        StandardsState.renderCount         = 0;

        StandardsState.view = 'lesson';
        ...
        if (glossaryFile) {
            try {
                ...
                console.warn('Course Explorer: found global/data/glossary.json but could not parse it as JSON — glossary will be empty.', e);
            }
        }
        ...
        try {
            ...
            console.warn('Course Explorer: failed to sort courseIndex — search results may appear in an unexpected order.', e);
        }
       ...
       // 1. Add terms that were found in the content
        ...
        try {
            ...
            console.warn('Course Explorer: failed to sort one or more media lists (video/audio/interactives/images) — dashboards may appear in an unexpected order.', e);
        }
        ...
        if (!wasRefreshed) hasRunMapConfig = false;
        isRefreshMode = false;
...
async function buildIndexAndHydrate(sitemap) {
    ...
    try {
        ...
        console.warn('Course Explorer: failed to sort courseStandardsList — the Standards tab may appear in an unexpected order.', e);
    }
}
    ...
    if (Array.isArray(obj)) {
        ...
        if (
            ...
            found.push({
                ...
                copyright:   obj.fieldValues.copyright || "",
                textVersion: textVer
            });
        }
