...
// Readability Dashboard state. Previously two separate globals
// (readabilityHierarchy in state.js, isAllReadabilityExpanded declared
// mid-file here); consolidated since readability.js is the only module that
// owns this feature. indexing.js resets `hierarchy` on each new course index
// via window.ReadabilityState, same as any other reference here.
//
// (state.js also had a `currentTargetGrade` global alongside readabilityHierarchy
// that turned out to be dead code -- declared but never read or written
// anywhere else in the app -- so it was dropped rather than carried forward.)
const ReadabilityState = {
    hierarchy:   [],   // built into a tree once a course is indexed; see buildReadabilityHierarchy()
    allExpanded: true, // table initializes in an expanded state
};
window.ReadabilityState = ReadabilityState;
...
window.buildReadabilityHierarchy = function() {
    ...
    courseTree.modules.forEach(mod => {
        ...
        if (!ReadabilityState.hierarchy.children[segKey]) {
            ReadabilityState.hierarchy.children[segKey] = {
                title: segKey, type: "segment",
                words: 0, sentences: 0, syllables: 0, longSentences: [],
                children: {}
            };
        }
        const segNode = ReadabilityState.hierarchy.children[segKey];
    ...
    courseIndex.forEach(item => {
        ...
        // Roll up to Course
        ReadabilityState.hierarchy.words += metrics.words; ReadabilityState.hierarchy.sentences += metrics.sentences;
        ReadabilityState.hierarchy.syllables += metrics.syllables; ReadabilityState.hierarchy.longSentences.push(...metrics.longSentences);
    });
    ...
    calcAgg(ReadabilityState.hierarchy);
};
...
window.renderReadabilityDashboard = function() {
    ...
    if (!ReadabilityState.hierarchy || !ReadabilityState.hierarchy.children || Object.keys(ReadabilityState.hierarchy.children).length === 0) {
    ...
    // Pass the entire course hierarchy as the root row, which will automatically recurse into its children!
        buildRow(ReadabilityState.hierarchy, 'r-level-course', 'root');
        container.innerHTML = html;

        // 1. Apply the global state as a baseline (catches any brand new rows if you changed the dropdown filter)
        if (typeof ReadabilityState.allExpanded !== 'undefined') {
            toggleAllReadability(ReadabilityState.allExpanded);
        }
...
// Table initializes in an expanded state (see ReadabilityState declaration at top of file)

window.toggleAllReadabilityState = function() {
    ReadabilityState.allExpanded = !ReadabilityState.allExpanded;
    const btn = document.getElementById('readability-toggle-all-btn');
    if (btn) {
        btn.innerHTML = ReadabilityState.allExpanded ? "Collapse All" : "Expand All";
    }
    toggleAllReadability(ReadabilityState.allExpanded);
};
...
window.exportReadabilityReport = function() {
    if (!ReadabilityState.hierarchy || !ReadabilityState.hierarchy.children) {
    ...
    try {
        ...
        Object.values(ReadabilityState.hierarchy.children).forEach(seg => addExportRow(seg, "Segment"));
