...
window.renderDownloadsList = function renderDownloadsList(query = '') {
    ...
    if (currentMediaLayout === 'list') {
        ...
        if (isUnlinked) {
            ...
            wrapper.innerHTML = `
                <table class="image-list-table">
                    <thead><tr><th>Location</th><th>Title</th><th>Filename</th><th>Type ${titleTypeCheckInfoButton()}</th><th>Structure Check ${structureCheckInfoButton()}</th><th>FLVS Footer ${footerCheckInfoButton()}</th></tr></thead>
                    <tbody>${tableRows}</tbody>
                </table>`;
        }
