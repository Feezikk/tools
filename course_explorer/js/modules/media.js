...
const MediaState = {
    ...
    audioBlobUrl:           null,
    scrollPositions:        {},    // tab -> { window: number, wrapper: number }, so switching tabs (or leaving/returning to the Media Dashboard) resumes where the user left off instead of snapping back to the top
    imageLoadedCount:       0,     // how many images have been progressively "Load Next"-ed into view in the Images tab; persists across tab switches so returning to Images resumes instead of restarting at the first batch
    imageBlobUrls:          [],    // object URLs currently backing rendered image thumbnails (grid + list), tracked so they can be revoked before the next render replaces them instead of leaking
};
window.MediaState = MediaState;

// Revokes every image-thumbnail blob URL created by the last Images render
// pass and clears the tracking list. Called right before any render that's
// about to throw away the DOM nodes referencing them (a fresh Images render,
// or the dashboard wiping #media-area to switch tabs/views) — otherwise
// each one created by URL.createObjectURL keeps its backing data alive in
// memory for the rest of the session even after its <img> is gone.
function revokePendingImageBlobUrls() {
    MediaState.imageBlobUrls.forEach(url => URL.revokeObjectURL(url));
    MediaState.imageBlobUrls = [];
}
window.revokePendingImageBlobUrls = revokePendingImageBlobUrls;

// Reads the scroll offset(s) for whatever is currently on screen for `tab`
// and stashes them for later. Two possible scroll containers exist: the
// page itself (grid/card layouts) and the internal table wrapper used by
// the list layout (`.image-list-wrapper`, which has its own overflow). We
// capture both unconditionally since it's cheap and means restoring doesn't
// need to know which one was active when the user navigated away.
function captureMediaScrollPosition(tab) {
    if (!tab) return;
    const wrapper = document.querySelector('#media-grid .image-list-wrapper');
    MediaState.scrollPositions[tab] = {
        window:  window.scrollY,
        wrapper: wrapper ? wrapper.scrollTop : 0
    };
}

// Re-applies a previously captured scroll offset for `tab`. Deferred two
// animation frames so it runs after the tab's content (including, for
// Images, any batches re-loaded to restore the previous count) has actually
// been laid out — scrolling immediately would just clamp to whatever little
// content exists synchronously after the innerHTML swap.
function restoreMediaScrollPosition(tab) {
    const saved = MediaState.scrollPositions[tab];
    if (!saved) return;
    requestAnimationFrame(() => {
        requestAnimationFrame(() => {
            window.scrollTo(0, saved.window || 0);
            const wrapper = document.querySelector('#media-grid .image-list-wrapper');
            if (wrapper) wrapper.scrollTop = saved.wrapper || 0;
        });
    });
}
window.captureMediaScrollPosition = captureMediaScrollPosition;
window.restoreMediaScrollPosition = restoreMediaScrollPosition;

window.setMediaTab = function setMediaTab(tab) {
    if (tab === MediaState.tab) return;
    ...
    restoreMediaScrollPosition(tab);
};
...
window.renderImagesList = function renderImagesList(query = '', opts = {}) {
    const { resume = false } = opts;
    MediaState.searchQuery = query;
    const grid = document.getElementById('media-grid');
    if (!grid) return;

    // Every image card/row about to be rebuilt below holds a blob URL from
    // the previous render — revoke those before creating their replacements.
    revokePendingImageBlobUrls();
    ...
    if (!filteredImages.length) {
        ...
        if (!resume) MediaState.imageLoadedCount = 0;
        return;
    }
    ...
    window.renderImageBatch = function() {
        ...
        for (let i = start; i < end; i++) {
            ...
            if (fileMap.has(cleanPath)) {
                ...
                MediaState.imageBlobUrls.push(srcUrl);
            }
        ...
        MediaState.imageLoadedCount = end;
        ...
        if (MediaState.renderCount < currentDisplayedMedia.length) {
            ...
            // For the list layout, the button needs to live *inside* the
            // scrollable .image-list-wrapper (alongside the table), not
            // after it in #media-grid. That wrapper is what actually
            // scrolls (it has its own max-height + overflow, with the
            // sticky header positioned relative to it) — every other tab's
            // table ends exactly at the bottom of that same wrapper, so
            // scrolling the window to the bottom reveals the whole thing
            // with the header still pinned in view. Appending the button
            // after the wrapper instead would stretch the page past it,
            // so reaching the true bottom would scroll the header (and the
            // rest of the table) out of view along with it.
            const listWrapperEl = MediaState.layout === 'list'
                ? document.querySelector('#media-grid .image-list-wrapper')
                : null;
            (listWrapperEl || grid).appendChild(btnWrapper);
        }
    };

    if (resume && MediaState.imageLoadedCount > MEDIA_BATCH_SIZE) {
        // Fast-forward through as many batches as the user had previously
        // loaded, instead of starting over at the first batch, so coming
        // back to the Images tab resumes rather than restarts.
        const target = Math.min(MediaState.imageLoadedCount, currentDisplayedMedia.length);
        while (MediaState.renderCount < target) {
...
window.renderVideosList = function renderVideosList(query = '') {
    ...
    if (MediaState.layout === 'list') {
        ...
        filteredVideos.forEach(vid => {
            ...
            const copyright = vid.copyright?.trim() ? escapeHtml(vid.copyright) : '&mdash;';
            ...
            tableRows += `
                <tr data-index="${idx}">
                    ...
                    <td style="white-space:normal;">${copyright}</td>
...
window.renderAudioList = function renderAudioList(query = '') {
    ...
    if (MediaState.layout === 'list') {
        ...
        filteredAudio.forEach(aud => {
            ...
            const copyright = aud.copyright?.trim() ? escapeHtml(aud.copyright) : '&mdash;';
...
window.renderMediaDashboard = function renderMediaDashboard(opts = {}) {
    const { resumeImages = false } = opts;
    const container = elements.mediaArea;

    // About to wipe out whatever's currently rendered — if that included
    // image thumbnails, release their blob URLs now rather than leaking
    // them (a fresh Images render, if that's where we're headed, will
    // create new ones).
    revokePendingImageBlobUrls();
    ...
    if (isGlossary) {
        ...
        renderImagesList(MediaState.searchQuery, { resume: resumeImages });
