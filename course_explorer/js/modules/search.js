...
// Content-search state (filters, active results, render pagination).
// Previously 12 separate globals in state.js; consolidated here since
// search.js is the only module that owns this feature. indexing.js resets
// these on each new course index, and import-export.js/app.js read a few of
// them (results/query/HTML-mode/exclude flags) when exporting or handling
// result-card clicks — all via window.SearchState, same as any reference here.
//
// Note: js/modules/search-worker.js's `_searchWorkerBody` function runs
// inside an actual Web Worker thread (a separate JS realm with no access to
// this window's globals at all) — it only exchanges plain data with the main
// thread via postMessage, so none of this state is or could be referenced
// from in there. Only `_handleWorkerResults` (the main-thread callback that
// receives the worker's reply) touches SearchState, same as any other
// consumer.
const SearchState = {
    activeModules:       new Set(),
    excludePages:        false,
    excludeInteractives: false,
...
// Standards feature state. This one's ownership is genuinely split across two
// files, so it's declared here (not in standards.js) for a load-order reason:
// search.js loads before standards.js, and several toggle functions wired in
// THIS file at top-level script-load time (toggleStdsModule, toggleStdsState,
// and standards.js's own toggleStdGroup/toggleAllStdGroups) capture these Sets
// as factory arguments immediately when the script runs -- so the object must
// already exist by the time either file executes, which means "whichever file
// loads first."
//
// In practice: search.js owns the Standards-Search filter dropdown UI +
// toggle wiring (activeModules/activeStates), while standards.js owns
// executing/rendering that search (view/groupedResults/renderCount/query/
// exactPattern/fuzzyPattern/searchResults/sharedColors/colorIndexTracker) AND
// the separate Curriculum-Map group filter (activeGroups, via
// toggleStdGroup/toggleAllStdGroups). indexing.js resets most fields on each
// new course index; dashboard.js/import-export.js read activeGroups;
// storage.js persists/restores activeGroups across sessions (always via
// .clear()/.add() in place -- see the comment in storage.js's
// restoreAppState for why reassigning this Set outright was a real bug).
const StandardsState = {
    // Standards Search tab
    activeModules:     new Set(),
    ...
    fuzzyPattern:       null,
    searchResults:      [],
    sharedColors:       new Map(),
    colorIndexTracker:  0,

    // Curriculum Map group filter
    activeGroups:       new Set(),
};
window.StandardsState = StandardsState;
...
window.renderSearchFilters = function renderSearchFilters() {
        ...
        modContainer.innerHTML =
            ...
            validModules.map(mod => {
                ...
                return buildFilterCheckboxHTML(
                    ...
                    `Module ${modNum}`,
                    SearchState.activeModules.has(modNum)
                );
            }).join('') +
            `</div>`;
        modBtn.innerHTML = SearchState.activeModules.size === 0
            ? `Modules <span id="svg-arrow-down-search-mod">${SVGS.arrowDown}</span>`
            : `${SearchState.activeModules.size} Selected <span id="svg-arrow-down-search-mod">${SVGS.arrowDown}</span>`;
        modBtn.classList.toggle('filtered', SearchState.activeModules.size > 0);
    }
    ...
    if (typeContainer && typeBtn) {
        typeContainer.innerHTML =
            `<div>` +
            buildFilterCheckboxHTML("toggleSearchExcludePages()",        'Exclude Course Pages', SearchState.excludePages) +
            buildFilterCheckboxHTML("toggleSearchExcludeInteractives()", 'Exclude Interactives', SearchState.excludeInteractives) +
            `<div style="border-top:1px dashed var(--border); margin-top:8px; padding-top:8px;">` +
            buildFilterCheckboxHTML("toggleSearchExcludeHTML()",      'Exclude HTML Code', SearchState.excludeHTML) +
            buildFilterCheckboxHTML("toggleSearchExcludeStandards()", 'Exclude Standards', SearchState.excludeStandards) +
            buildFilterCheckboxHTML("toggleSearchExcludeML()",        'Exclude ML',        SearchState.excludeML) +
            buildFilterCheckboxHTML("toggleSearchExcludeNotes()",     'Exclude Notes',     SearchState.excludeNotes) +
            `</div></div>`;

        const isNonDefault =
            SearchState.excludePages ||
            SearchState.excludeInteractives ||
            !SearchState.excludeHTML ||
            !SearchState.excludeStandards ||
            !SearchState.excludeML ||
            !SearchState.excludeNotes;
...
window.toggleSearchModule = makeSetToggle(SearchState.activeModules, () => {
    renderSearchFilters();
    if (elements.searchInput.value.trim()) runSearch(elements.searchInput.value);
});

window.toggleAllSearchModules = makeSetAllToggle(
    SearchState.activeModules,
...
window.toggleAllSearchTypes = function toggleAllSearchTypes(state) {
    if (state) {
        // Reset to default
        SearchState.excludePages          = false;
        SearchState.excludeInteractives   = false;
        SearchState.excludeHTML           = true;
        SearchState.excludeStandards      = true;
        SearchState.excludeML             = true;
        SearchState.excludeNotes          = true;
    } else {
        // "Clear All" unchecks everything
        SearchState.excludePages          = false;
        SearchState.excludeInteractives   = false;
        SearchState.excludeHTML           = false;
        SearchState.excludeStandards      = false;
        SearchState.excludeML             = false;
        SearchState.excludeNotes          = false;
    }
    renderSearchFilters();
    if (elements.searchInput.value.trim()) runSearch(elements.searchInput.value);
};
...
window.toggleSearchExcludePages        = makeFlagToggle(() => SearchState.excludePages,        v => SearchState.excludePages = v,        onSearchFilterChange);
window.toggleSearchExcludeInteractives = makeFlagToggle(() => SearchState.excludeInteractives, v => SearchState.excludeInteractives = v, onSearchFilterChange);
window.toggleSearchExcludeHTML         = makeFlagToggle(() => SearchState.excludeHTML,          v => SearchState.excludeHTML = v,          onSearchFilterChange);
window.toggleSearchExcludeStandards    = makeFlagToggle(() => SearchState.excludeStandards,     v => SearchState.excludeStandards = v,     onSearchFilterChange);
window.toggleSearchExcludeML           = makeFlagToggle(() => SearchState.excludeML,             v => SearchState.excludeML = v,             onSearchFilterChange);
window.toggleSearchExcludeNotes        = makeFlagToggle(() => SearchState.excludeNotes,          v => SearchState.excludeNotes = v,          onSearchFilterChange);
        ...
        modContainer.innerHTML =
            ...
            validModules.map(mod => {
                ...
                return buildFilterCheckboxHTML(
                    ...
                    `Module ${modNum}`,
                    StandardsState.activeModules.has(modNum)
                );
            }).join('') +
            `</div>`;
        modBtn.innerHTML = StandardsState.activeModules.size === 0
            ? `Modules <span id="svg-arrow-down-stds-mod">${SVGS.arrowDown}</span>`
            : `${StandardsState.activeModules.size} Selected <span id="svg-arrow-down-stds-mod">${SVGS.arrowDown}</span>`;
        modBtn.classList.toggle('filtered', StandardsState.activeModules.size > 0);
    }
        ...
        stateContainer.innerHTML =
            ...
            Array.from(courseStandardGroups).sort().map(g =>
                buildFilterCheckboxHTML(
                    ...
                    escapeHtml(g),
                    StandardsState.activeStates.has(g)
                )
            ).join('') +
            `</div>`;
        stateBtn.innerHTML = StandardsState.activeStates.size === 0
            ? `States <span id="svg-arrow-down-stds-state">${SVGS.arrowDown}</span>`
            : `${StandardsState.activeStates.size} Selected <span id="svg-arrow-down-stds-state">${SVGS.arrowDown}</span>`;
        stateBtn.classList.toggle('filtered', StandardsState.activeStates.size > 0);
    }
};

const onStdsFilterChange = () => {
    renderStdsFilters();
    runStdsSearch();
};

window.toggleStdsModule = makeSetToggle(StandardsState.activeModules, onStdsFilterChange);

window.toggleAllStdsModules = makeSetAllToggle(
    StandardsState.activeModules,
    () => validModules.map(mod => padNum(mod.num)),
    onStdsFilterChange
);

window.toggleStdsState = makeSetToggle(StandardsState.activeStates, onStdsFilterChange);

window.toggleAllStdsStates = makeSetAllToggle(
    StandardsState.activeStates,
    () => Array.from(courseStandardGroups),
    onStdsFilterChange
);
...
window.runSearch = function runSearch(query) {
    ...
    SearchState.results       = [];
    SearchState.query         = q;
    SearchState.isHtmlMode    = elements.htmlToggle.checked;
    SearchState.renderCount          = 0;
    ...
    const options = {
        ...
        excludePages:        SearchState.excludePages,
        excludeInteractives: SearchState.excludeInteractives,
        excludeHTML:         SearchState.excludeHTML,
        excludeML:           SearchState.excludeML,
        excludeStds:         SearchState.excludeStandards,
        excludeNotes:        SearchState.excludeNotes,
        activeModules:       Array.from(SearchState.activeModules)
    };
...
function renderSearchBatch() {
    ...
    const start = SearchState.renderCount;
    const end   = Math.min(start + SEARCH_BATCH_SIZE, SearchState.results.length);

    for (let i = start; i < end; i++) {
        frag.appendChild(
            createResultCard(SearchState.results[i], SearchState.query, SearchState.isHtmlMode, i)
        );
    }

    document.getElementById('load-more-search-btn')?.remove();
    elements.results.appendChild(frag);
    SearchState.renderCount = end;

    if (SearchState.renderCount < SearchState.results.length) {
        const remaining = SearchState.results.length - SearchState.renderCount;
        ...
        btn.innerHTML =
            ...
            `(Showing ${SearchState.renderCount} of ${SearchState.results.length})`;
...
function getSnippet(fullText, index, length, query, isHtmlMode, matchType = 'normal') {
    ...
    if (matchLength > 0 && query && SearchState.highlightRegex) {
        textPart = textPart.replace(
            SearchState.highlightRegex,
...
window.openPageModal = async function openPageModal(
    ...
    // Use SearchState.highlightRegex from AST instead of raw string regex matching
    let rawRegex = SearchState.highlightRegex;
            ...
            if (rawRegex) {
                ...
                function injectTokens(o, ctx) {
                    ...
                    if (typeof o === 'object') {
                        const result = {};
                        for (const key in o) {
                            if (SearchState.excludeML        && (key === 'ml'        || key === 'chatHistory')) { result[key] = o[key]; continue; }
                            if (SearchState.excludeStandards && (key === 'standards' || key === 'standard'))    { result[key] = o[key]; continue; }
                            if (SearchState.excludeNotes     && (key === 'notes'     || key === 'note'))        { result[key] = o[key]; continue; }
    ...
    document.getElementById('modal-header-content').innerHTML = `
        <h3 class="flex-center gap-10"
            ...
            ${SVGS.fileText} ${escapeHtml(res.filename)}
        </h3>
        <div style="font-size:0.85rem; color:var(--text-light);">
            <span class="id-number"># ${escapeHtml(res.locationId)}</span>
            &nbsp;|&nbsp;
            <span>${escapeHtml(res.breadcrumb)}</span>
        </div>
