...
window.updateStickyHeaderOffset = function updateStickyHeaderOffset() {
        const mapHeader = document.querySelector('#map-detail-area .sticky-top-wrapper');
        if (mapHeader && mapHeader.offsetParent !== null) {
            const h = mapHeader.getBoundingClientRect().height;
...
window.toggleFilterDropdown = function toggleFilterDropdown(id, event) {
    ...
    if (id === 'media-filter-dropdown') MediaState.isFilterOpen = !wasShowing;
};
