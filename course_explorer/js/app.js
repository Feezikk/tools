...
document.addEventListener("DOMContentLoaded", () => {
    const svgInjections = {
        ...
        'svg-arrow-down-map-state': SVGS.arrowDown,
    ...
    document.getElementById('btn-view-map').innerHTML  = `${SVGS.map}       Curriculum Map`;
    ...
    document.getElementById('reconfig-btn').innerHTML    = `${SVGS.config}    Configure Map`;
    ...
    elements.gapToggle.addEventListener('change', (e) => {
        if (e.target.checked) {
            elements.mapDetailArea.classList.add('gap-mode');
            isAllExpanded = true;
            updateToggleButton();
            applyExpansionState(true);
        } else {
            elements.mapDetailArea.classList.remove('gap-mode');
        }
        saveAppState();
    });

    elements.mapFilterRadios.forEach(radio => {
        radio.addEventListener('change', (e) => {
            elements.mapDetailArea.classList.remove('show-objs-only', 'show-stds-only');
            if      (e.target.value === 'obj') elements.mapDetailArea.classList.add('show-objs-only');
            else if (e.target.value === 'std') elements.mapDetailArea.classList.add('show-stds-only');
            saveAppState();
    ...
    elements.results.addEventListener('click', (e) => {
        ...
        if (!card) return;
        const res = SearchState.results[card.dataset.index];
        if (!res) return;
        ...
        openPageModal(
            res, SearchState.query, SearchState.isHtmlMode,
    ...
    elements.mediaArea.addEventListener('click', (e) => {
        const downloadBtn = e.target.closest('.download-icon-btn');
        if (downloadBtn) {
            const entry = currentDisplayedMedia[downloadBtn.dataset.index];
            if (entry) openDocumentFile(entry.item.path);
            return;
        }
        ...
        if (imgCard && imgCard.dataset.index) {
            ...
            if (entry && entry.type !== 'document') openMediaModalFromObj(entry.item, entry.type);
            return;
        }
